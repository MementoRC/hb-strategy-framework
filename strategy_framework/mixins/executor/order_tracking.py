...
class OrderTrackingMixin:
    ...
    @property
    def open_orders(self: OrderTrackingProtocol) -> list[TrackedOrderProtocol]:
        """List of currently open (entry) orders."""
        return cast("list[TrackedOrderProtocol]", self._open_orders)  # type: ignore[attr-defined]

    @property
    def close_orders(self: OrderTrackingProtocol) -> list[TrackedOrderProtocol]:
        """List of close (exit) orders."""
        return cast("list[TrackedOrderProtocol]", self._close_orders)  # type: ignore[attr-defined]

    def add_open_order(
        self: OrderTrackingProtocol,
    ...
    def get_filled_open_orders(
        ...
        orders = cast("list[TrackedOrderProtocol]", self._open_orders)  # type: ignore[attr-defined]
